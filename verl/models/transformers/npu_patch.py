...
class NPUQwen3MoeExperts(nn.Module):
    ...
    def forward(
        ...
        up_res = NPUGmmFunction.apply(permuted_tokens, self.up_proj.transpose(1, 2), tokens_per_expert)
        gate_res = NPUGmmFunction.apply(permuted_tokens, self.gate_proj.transpose(1, 2), tokens_per_expert)
        act_res = torch_npu.npu_swiglu(torch.cat([gate_res, up_res], dim=-1))
        down_res = NPUGmmFunction.apply(act_res, self.down_proj.transpose(1, 2), tokens_per_expert)
